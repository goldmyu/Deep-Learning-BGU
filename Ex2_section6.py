...
def cnn_model(features, labels, mode):
    ...
    tf.summary.scalar('accuracy', accuracy_tb)
    logging_hook = tf.train.LoggingTensorHook({"loss": loss,
                                               "accuracy": accuracy_tb}, every_n_iter=250)
...
def main(unused_argv):
    ...
    mnist_classifier = tf.estimator.Estimator(model_fn=cnn_model, model_dir='/tensor/log',  config=run_config)
